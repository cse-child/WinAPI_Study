...
{
    Texture * upPipeTexture = TEXTURE->Add(L"Textures/pipeup.bmp", 26, 162);
    Texture * downPipeTexture = TEXTURE->Add(L"Textures/pipedown.bmp", 26, 162);
    ...
    for (UINT i = 0; i < PIPE_NUM; i++)
    {
        ...
        downPipes[i]->Pos().y =
            upPipes[i]->Pos().y + interval.y + upPipes[i]->Size().y;
    }
}
...
void PipeManager::Update()
{
    for (UINT i = 0; i < PIPE_NUM; i++)
    {
        ...
        {
            upPipes[i]->Pos().x += (upPipes[i]->Half().x + interval.x) * (PIPE_NUM - 1);
            downPipes[i]->Pos().x += (upPipes[i]->Half().x + interval.x) * (PIPE_NUM - 1);
